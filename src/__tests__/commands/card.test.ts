...
describe('card commands', () => {
  ...
  test('card create sends expected payload', async () => {
    ...
    mockServer = Bun.serve({
      ...
      async fetch(request) {
        ...
        if (request.method === 'POST' && url.pathname === '/api/v1/cards') {
    ...
    expect(captured?.pathname).toBe('/api/v1/cards');
  ...
  test('card get supports json output', async () => {
    mockServer = Bun.serve({
      ...
      fetch(request) {
        const url = new URL(request.url);
        if (request.method === 'GET' && url.pathname === '/api/v1/cards/card_1') {
          return new Response(JSON.stringify({
            id: 'card_1',
            agentId: 'agent_1',
            label: 'Primary',
  ...
  test('card update sends expected payload', async () => {
    ...
    mockServer = Bun.serve({
      ...
      async fetch(request) {
        const url = new URL(request.url);
        if (request.method === 'PUT' && url.pathname === '/api/v1/cards/card_1') {
          const body = await request.json();
          capturedRequests.push({
            method: request.method,
            pathname: url.pathname,
    ...
    const captured = capturedRequests[0];
    expect(captured?.pathname).toBe('/api/v1/cards/card_1');
    expect(captured?.body).toEqual({
      label: 'Updated',
      status: 'FROZEN',
      spendLimits: {
  ...
  test('card delete hits delete endpoint', async () => {
    ...
    mockServer = Bun.serve({
      ...
      fetch(request) {
        const url = new URL(request.url);
        if (request.method === 'DELETE' && url.pathname === '/api/v1/cards/card_1') {
          capturedRequests.push({
            method: request.method,
            pathname: url.pathname,
            searchParams: url.searchParams,
  ...
  test('card transactions sends expected query', async () => {
    ...
    mockServer = Bun.serve({
      ...
      fetch(request) {
        const url = new URL(request.url);
        if (request.method === 'GET' && url.pathname === '/api/v1/cards/transactions') {
          capturedRequests.push({
            method: request.method,
            pathname: url.pathname,
            searchParams: url.searchParams,
    ...
    const captured = capturedRequests[0];
    expect(captured?.pathname).toBe('/api/v1/cards/transactions');
  ...
  test('card kill-switch sends expected payload', async () => {
    ...
    mockServer = Bun.serve({
      ...
      async fetch(request) {
        const url = new URL(request.url);
        if (request.method === 'POST' && url.pathname === '/api/v1/cards/kill-switch') {
          const body = await request.json();
          capturedRequests.push({
            method: request.method,
            pathname: url.pathname,
    ...
    const captured = capturedRequests[0];
    expect(captured?.pathname).toBe('/api/v1/cards/kill-switch');
    expect(captured?.body).toEqual({
      active: true,
      cardId: 'card_1',
    });
